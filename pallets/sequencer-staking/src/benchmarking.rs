// command: 
// cargo run benchmark pallet --execution=wasm --wasm-execution=compiled --pallet pallet-sequencer-staking --extrinsic * --steps 50 --repeat 20 --template=./benchmarking/frame-weight-template.hbs --json-file raw.json --output weights.rs
...
/// Minimum sequencer candidate stake
...
enum AccountAction<T: Config> {
	...
	Delegate {
		...
		sequencer_delegation_count: u32,
		sequencer_auto_compound_delegation_count: u32,
	},
	JoinCandidates {
		amount: Amount<BalanceOf<T>>,
		candidate_count: u32,
...
fn create_account<T: Config>(
	...
	match action {
		...
		AccountAction::Delegate {
			...
			amount,
			auto_compound,
			sequencer_delegation_count,
			sequencer_auto_compound_delegation_count,
		} => {
...
/// Create a funded sequencer.
fn create_funded_sequencer<T: Config>(
	string: &'static str,
	n: u32,
	extra: BalanceOf<T>,
	min_bond: bool,
...
benchmarks! {
	// ROOT DISPATCHABLESs
    charge_reward_account {
        let (caller, _) = create_funded_user::<T>("caller", USER_SEED, 0u32.into());
    }: _(RawOrigin::Signed(caller), 1200u32)
	verify {
        let pallet_account = Pallet::<T>::account_id();
        assert!(
			T::Currency::free_balance(&pallet_account) == 1200u32.into(),
			"Reward account should be charged"
		);
	}
	...
	set_sequencer_commission {}: _(RawOrigin::Root, Perbill::from_percent(33))
	verify {
		assert_eq!(Pallet::<T>::sequencer_commission(), Perbill::from_percent(33));
	}
	...
	join_candidates {
		...
		for i in 2..x {
			...
			let sequencer = create_funded_sequencer::<T>(
				"sequencer",
				seed,
				0u32.into(),
				true,
	...
	// This call schedules the sequencer's exit and removes them from the candidate pool
	...
	schedule_leave_candidates {
		...
		let caller: T::AccountId = create_funded_sequencer::<T>(
			"caller",
			USER_SEED,
			0u32.into(),
			true,
	...
	execute_leave_candidates_worst_case {
		...
		let candidate: T::AccountId = create_funded_sequencer::<T>(
			"unique_caller",
			USER_SEED - 100,
			0u32.into(),
			true,
		...
		let second_candidate: T::AccountId = create_funded_sequencer::<T>(
			"unique__caller",
			USER_SEED - 99,
			0u32.into(),
			true,
	...
	delegate {
		...
		let mut sequencers: Vec<T::AccountId> = Vec::new();
		// Initialize MaxDelegationsPerDelegator sequencer candidates
		for i in 2..x {
			let seed = USER_SEED - i;
			let sequencer = create_funded_sequencer::<T>(
				"sequencer",
				...
				0u32.into(),
				true,
				sequencers.len() as u32 + 1u32,
			)?;
			sequencers.push(sequencer.clone());
		}
		let bond = <<T as Config>::MinDelegation as Get<BalanceOf<T>>>::get();
		let extra = if (bond * (sequencers.len() as u32 + 1u32).into()) > min_candidate_stk::<T>() {
			(bond * (sequencers.len() as u32 + 1u32).into()) - min_candidate_stk::<T>()
		} else {
			0u32.into()
		};
		...
		// Nominate MaxDelegationsPerDelegators sequencer candidates
		for col in sequencers.clone() {
			Pallet::<T>::delegate(
				RawOrigin::Signed(caller.clone()).into(), col, bond, 0u32, del_del_count
			)?;
			del_del_count += 1u32;
		}
		// Last sequencer to be delegated
		let sequencer: T::AccountId = create_funded_sequencer::<T>(
			"sequencer",
			USER_SEED,
			0u32.into(),
			true,
		...
		// Worst Case Complexity is insertion into an almost full sequencer
		let mut col_del_count = 0u32;
		for i in 1..y {
			let seed = USER_SEED + i;
	...
	}: _(RawOrigin::Signed(caller.clone()), sequencer, bond, col_del_count, del_del_count)
	verify {
	...
	schedule_revoke_delegation {
		...
		let sequencer = create_account::<T>(
			"sequencer",
			seed.take(),
		...
		for i in 0..num_top {
			let del = create_account::<T>(
				...
				AccountAction::Delegate{
					...
					sequencer_delegation_count: col_del_count,
					sequencer_auto_compound_delegation_count: col_del_count,
				},
			)?;
			col_del_count += 1;
		...
		assert_eq!(
			<BottomDelegations<T>>::get(&sequencer).map(|d| d.delegations.len()).unwrap_or_default(),
			0,
		);
		...
		assert_eq!(
			<BottomDelegations<T>>::get(&sequencer).map(|bd| bd.delegations.len() as u32).unwrap_or_default(),
			num_bottom,
		);
	...
	verify {
		...
		assert_eq!(
			Pallet::<T>::delegation_scheduled_requests(&sequencer)
				.iter()
	...
	execute_revoke_delegation {
		...
		roll_to_and_author::<T>(2, sequencer.clone());
	}: {
	...
	execute_delegator_revoke_delegation_worst {
		// We assume delegator has auto-compound set, sequencer has max scheduled requests, and delegator
		...
		assert!(
			<BottomDelegations<T>>::get(&sequencer)
		...
		assert!(
			<TopDelegations<T>>::get(&sequencer)
				.map(|bd| bd.delegations.iter().any(|d| d.owner == highest_bottom_delegator))
				.unwrap_or_default(),
		);
	}
	...
	execute_delegator_bond_less_worst {
		// We assume delegator will be kicked into bottom delegation and sequencer has
	...
	}: { _results = Some(Pallet::<T>::get_rewardable_delegators(&sequencer)); }
	verify {
		...
		let top_delegations = <TopDelegations<T>>::get(sequencer.clone())
			.expect("delegations were set for sequencer through delegate() calls");
		assert!(top_delegations.delegations.len() as u32 == y);
	}

	select_top_candidates {
		let x in 0..50; // num sequencers
		let y in 0..<<T as Config>::MaxDelegationsPerDelegator as Get<u32>>::get(); // num delegators
	...
	pay_one_sequencer_reward_best {
		...
		// z is the number of scheduled requests per sequencer
		let z in 0..(
			T::MaxTopDelegationsPerCandidate::get()
			+ T::MaxBottomDelegationsPerCandidate::get() - 1
		);
		...
		use crate::{
			DelayedPayout, DelayedPayouts, AtStake, SequencerSnapshot, BondWithAutoCompound, Points,
			AwardedPts,
		};
		...
		for i in 0..x {
			...
			let delegator = create_account::<T>(
				...
				AccountAction::Delegate{
					sequencer: prime_candidate.clone(),
					amount: Amount::All,
					auto_compound,
					sequencer_delegation_count: col_del_count,
					sequencer_auto_compound_delegation_count: col_del_count,
		...
		<DelayedPayouts<T>>::insert(&round_for_payout, DelayedPayout {
			...
			sequencer_commission: Perbill::from_rational(1u32, 100u32),
		});

		<AtStake<T>>::insert(round_for_payout, &prime_candidate, SequencerSnapshot {
			bond: 1_000u32.into(),
	...
	verify {
		...
		} in &delegations
		{
			assert!(
				...
				"delegator should have been paid in pay_one_sequencer_reward"
			);
		}
	}

	pay_one_sequencer_reward {
		// y controls number of delegations, its maximum per sequencer is the max top delegations
		...
		// initialize our single sequencer
		let sole_sequencer = create_funded_sequencer::<T>(
			"sequencer",
			0,
		...
		// directly and then call pay_one_sequencer_reward directly.
		...
		<AtStake<T>>::insert(round_for_payout, &sole_sequencer, SequencerSnapshot {
			bond: 1_000u32.into(),
			delegations,
			total: 1_000_000u32.into(),
		});
		...
		<Points<T>>::insert(round_for_payout, 100);
		<AwardedPts<T>>::insert(round_for_payout, &sole_sequencer, 20);
		...
		let result = Pallet::<T>::pay_one_sequencer_reward(round_for_payout, payout_info);
		// TODO: how to keep this in scope so it can be done in verify block?
		assert!(matches!(result.0, RewardPayment::Paid));
	}
	verify {
		// sequencer should have been paid
		assert!(
			T::Currency::free_balance(&sole_sequencer) > initial_stake_amount,
			"sequencer should have been paid in pay_one_sequencer_reward"
		);
	...
	base_on_initialize {
		...
		parachain_staking_on_finalize::<T>(sequencer.clone());
	...
	set_auto_compound {
		// x controls number of distinct auto-compounding delegations the prime sequencer will have
		...
		// initialize the prime sequencer
		let prime_candidate = create_funded_sequencer::<T>(
			"sequencer",
			seed.take(),
			min_candidate_stake,
			true,
		...
		// have x-1 distinct auto-compounding delegators delegate to prime sequencer
		...
		// delegate to y-1 distinct sequencers from the prime delegator
		for i in 1..y {
			let sequencer = create_funded_sequencer::<T>(
				"sequencer",
				seed.take(),
	...
	delegate_with_auto_compound {
		// x controls number of distinct delegations the prime sequencer will have
		// y controls number of distinct auto-compounding delegations the prime sequencer will have
		...
		// have x-1 distinct delegators delegate to prime sequencer, of which y are auto-compounding.
		...
		// delegate to z-1 distinct sequencers from the prime delegator
		for i in 1..z {
			let sequencer = create_funded_sequencer::<T>(
				"sequencer",
				seed.take(),
	...
	delegate_with_auto_compound_worst {
		...
		// setup max delegations on prime sequencer, with a bottom delegation that will be kicked and
		// has scheduled requests on different sequencers.
		let mut decreasing_balance = <DecreasingBalance<T>>::new(
			T::MinDelegation::get() * 2000u32.into(),
			1u32.into(),
		);
	...
	mint_sequencer_reward {
		...
		let original_free_balance = T::Currency::free_balance(&sequencer);
	}: {
		Pallet::<T>::mint_sequencer_reward(1u32.into(), sequencer.clone(), 50u32.into())
	}
	verify {
		assert_eq!(T::Currency::free_balance(&sequencer), original_free_balance + 50u32.into());
	}

	notify_inactive_sequencer {
		use crate::{AtStake, SequencerSnapshot, AwardedPts};
		...
		// Create sequencers up to MaxCandidates
		...
		// Create two sequencers more: the one that will be marked as inactive
		// and the one that will act as the caller of the extrinsic.
		seed += 1;
		let inactive_sequencer: T::AccountId = create_funded_sequencer::<T>(
			"sequencer",
			seed,
			min_candidate_stk::<T>() * 1_000_000u32.into(),
			true,
		...
		// Manually change these values for inactive_sequencer,
		// so that it can be marked as inactive.
		<AtStake<T>>::insert(1, &inactive_sequencer, SequencerSnapshot::default());
		<AwardedPts<T>>::insert(1, &inactive_sequencer, 0);

		<AtStake<T>>::insert(2, &inactive_sequencer, SequencerSnapshot::default());
		<AwardedPts<T>>::insert(2, &inactive_sequencer, 0);

		// Enable killswitch
		<EnableMarkingOffline<T>>::set(true);

	}: _(RawOrigin::Signed(caller), inactive_sequencer.clone())
	verify {
		assert!(!Pallet::<T>::candidate_info(&inactive_sequencer).expect("must exist").is_active());
	}
}
...
mod tests {
	...
	#[test]
	fn bench_set_sequencer_commission() {
		new_test_ext().execute_with(|| {
			assert_ok!(Pallet::<Test>::test_benchmark_set_sequencer_commission());
		});
	}
