...
use crate::{
	...
	service::{new_partial, PopsicleRuntimeExecutor},
};
...
fn load_spec(id: &str) -> std::result::Result<Box<dyn ChainSpec>, String> {
	Ok(match id {
		"" | "popsicle" | "popsicle-dev" => Box::new(chain_spec::popsicle::development_config()),
		"popsicle-local" => Box::new(chain_spec::popsicle::local_testnet_config()),
		path => {
		...
		match runner.config().chain_spec.runtime() {
			Runtime::Popsicle | Runtime::Default => {
				runner.async_run(|$config| {
					let $components = new_partial::<popsicle_runtime::RuntimeApi, PopsicleRuntimeExecutor, _>(
						&$config,
						crate::service::build_import_queue::<popsicle_runtime::RuntimeApi, PopsicleRuntimeExecutor>,
					)?;
...
macro_rules! construct_benchmark_partials {
	($config:expr, |$partials:ident| $code:expr) => {
		match $config.chain_spec.runtime() {
			Runtime::Popsicle | Runtime::Default => {
				let $partials =
					new_partial::<popsicle_runtime::RuntimeApi, PopsicleRuntimeExecutor, _>(
						&$config,
						crate::service::build_import_queue::<_, PopsicleRuntimeExecutor>,
					)?;
				$code
			},
		}
	...
	match &cli.subcommand {
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				match config.chain_spec.runtime() {
					Runtime::Default | Runtime::Popsicle => {
						sp_core::crypto::set_default_ss58_version(
							popsicle_runtime::SS58Prefix::get().into(),
						);
						crate::service::start_parachain_node::<
							popsicle_runtime::RuntimeApi,
							PopsicleRuntimeExecutor,
